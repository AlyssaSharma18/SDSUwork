...
fitter_opts_PL = Dict(
    ...
    :boundsMH => [eps(Float64) eps(Float64) eps(Float64); 2. 2. 150.0]
)
...
# beta_h
theta1, sol1 = ProfileLikelihood.PL(1e-5, 500, 1, paramsFitted, [noisyData], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5])
# theta1, sol1 = ProfileLikelihood.PL(5e-4, 10, 1, paramsFitted, [noisyData], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5], status = "global")
sol1 = sol1 .+ PLconst 
...
# beta_v (decrease step size and increase number of steps yo!)
theta2, sol2 = ProfileLikelihood.PL(1.125e-3, 85, 2, paramsFitted, [noisyData], [], threshold + 0.5, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5], status = "global")
sol2 = sol2 .+ PLconst 
deleteat!(theta2, 1)
deleteat!(sol2, 1)
...
# gamma
theta3, sol3 = ProfileLikelihood.PL(1e-1, 1050, 3, paramsFitted, [noisyData], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5])
# theta3, sol3 = ProfileLikelihood.PL(2, 10, 3, paramsFitted, [noisyData], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5], status = "global")
sol3 = sol3 .+ PLconst 
...
jldsave("sisIncidencePoisson0_15.jld2"; loss, paramsFitted, threshold, thresholdP, PLconst, theta1, sol1, theta2, sol2, theta3, sol3)