...
# Plot solution 
plt = plot(times, perfectDataHost, dpi = 400, legend = :topleft, labels = "Perfect Incidence Host Data", xlabel = L"t", ylabel = "Incidence (Host)")
plot!(times, noisyDataHost, labels = "Noisy Incidence Host Data")
display(plt)
savefig(plt, "perfectNoisyHostData")
plt1 = plot(times, perfectDataVector, dpi = 400, legend = :topleft, labels = "Perfect Incidence Vector Data", xlabel = L"t", ylabel = "Incidence (Vector)")
plot!(times, noisyDataVector, labels = "Noisy Incidence Vector Data")
display(plt1)
savefig(plt1, "perfectNoisyVectorData")
...
p1 = [9.329466810800791e-5, 0.0010666198022741937, 0.09775869214627872]
...
fitter_opts_PL = Dict(
    :algFirst => :generating_set_search,
    :searchRange => [(eps(Float64), 3.0), (eps(Float64), 3.0), (eps(Float64), 3.0)], 
    :maxTimeFirst => 65.0,
    :traceMode => :compact,
    :algNLOpt => :LD_TNEWTON,
...
# beta_h
theta1, sol1 = ProfileLikelihood.PL(8.333e-7, 60, 1, paramsFitted, [noisyDataHost, noisyDataVector], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj, obj]; incidenceObserved = [5, 6], status = "globalBB")
sol1 = sol1 .+ PLconst 
...
# theta11, sol11 = ProfileLikelihood.PL(5e-6, 10, 1, paramsFitted, [noisyDataHost, noisyDataVector], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj, obj]; incidenceObserved = [5, 6], status = "global")
# sol11 = sol11 .+ PLconst 
...
# beta_v
theta2, sol2 = ProfileLikelihood.PL(5e-6, 55, 2, paramsFitted, [noisyDataHost, noisyDataVector], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj, obj]; incidenceObserved = [5, 6], status = "globalBB")
sol2 = sol2 .+ PLconst 
...
# theta21, sol21 = ProfileLikelihood.PL(5e-5, 5, 2, paramsFitted, [noisyDataHost, noisyDataVector], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj, obj]; incidenceObserved = [5, 6], status = "global")
# sol21 = sol21 .+ PLconst 
...
# gamma
theta3, sol3 = ProfileLikelihood.PL(1e-3, 55, 3, paramsFitted, [noisyDataHost, noisyDataVector], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj, obj]; incidenceObserved = [5, 6], status = "globalBB")
sol3 = sol3 .+ PLconst 
...
# theta31, sol31 = ProfileLikelihood.PL(5e-3, 10, 3, paramsFitted, [noisyDataHost, noisyDataVector], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj, obj]; incidenceObserved = [5, 6], status = "global")
# sol31 = sol31 .+ PLconst 
...
jldsave("sisIncidenceMoreObservationsPoisson0_30.jld2"; loss, paramsFitted, threshold, thresholdP, PLconst, theta1, sol1, theta2, sol2, theta3, sol3)