...
# Initial guess that was calculated from repeated trials of optimization, 527.273 and minimum on graph is 1109.683976823765
p1 = [0.000103298, 0.000973206, 0.0911456, 4.89737e-16, 0.0911663]
...
plt = plot(times, sol[1,:], legend=:topleft, labels = "Fitted Parameters", xlabel = L"t", ylabel = L"I_h", dpi = 400)
scatter!(times, perfectData, labels = "Perfect Data")
scatter!(times, noisyData, labels = "Noisy Data")
display(plt)

# Constants to add back 
PLconst = ProfileLikelihood.likelihoodConst("relativeError"; noiseLevel = 0.01, times = times)
println(PLconst)

# Graph profile likelihood 
theta, sol = ProfileLikelihood.PL(0.0000008, 100, 1, paramsFitted, [noisyData], [1], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj])
sol = sol .+ PLconst 
...
# theta3, sol3 = ProfileLikelihood.PL(0.00002, 600, 4, paramsFitted, [noisyData], [1], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj])
# sol3 = sol3 .+ PLconst 
...
# theta1, sol1 = ProfileLikelihood.PL(0.000006, 210, 2, paramsFitted, [noisyData], [1], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj])
# sol1 = sol1 .+ PLconst
...
# theta2, sol2 = ProfileLikelihood.PL(0.001, 400, 3, paramsFitted, [noisyData], [1], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj])
# sol2 = sol2 .+ PLconst
...
# theta4, sol4 = ProfileLikelihood.PL(0.0001, 400, 5, paramsFitted, [noisyData], [1], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj])
# sol4 = sol4 .+ PLconst
