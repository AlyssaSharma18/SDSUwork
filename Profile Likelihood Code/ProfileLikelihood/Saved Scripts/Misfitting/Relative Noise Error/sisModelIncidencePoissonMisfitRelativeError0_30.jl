...
fitter_opts_PL = Dict(
    ...
    :boundsMH => [eps(Float64) eps(Float64) eps(Float64); 0.5 0.5 10.]
)

# Constants to add back 
PLconst = ProfileLikelihood.likelihoodConst("relativeError"; times = times, noiseLevel = 0.3)
println("The profile likelihood constant is ", PLconst)

# beta_h (global)
theta1, sol1 = ProfileLikelihood.PL(3.33e-6, 120, 1, paramsFitted, [noisyData], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5], status = "globalBB")
sol1 = sol1 .+ PLconst 
...
# beta_v 
theta2, sol2 = ProfileLikelihood.PL(8e-4, 90, 2, paramsFitted, [noisyData], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5], status = "globalBB")
sol2 = sol2 .+ PLconst 
...
# gamma
theta3, sol3 = ProfileLikelihood.PL(2.7e-1, 120, 3, paramsFitted, [noisyData], [], threshold + 3, loss, prob, solver_opts, times, fitter_opts_PL, [obj]; incidenceObserved = [5], status = "globalBB")
sol3 = sol3 .+ PLconst 
...
# jldsave("sisIncidencePoissonMisfitRelativeError0_30.jld2"; loss, paramsFitted, threshold, thresholdP, PLconst, theta1, sol1, theta2, sol2, theta3, sol3)