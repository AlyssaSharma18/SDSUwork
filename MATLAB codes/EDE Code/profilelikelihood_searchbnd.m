...
newparam = EstiParam(50,:);
paramtest = [0.00027 0.01 0.01 0.00005 0.1 0.074 0.2 0.0067 0.00018 0.000033 0.0067];
...
disp(err_in_data_SIR(paramtest,Prevalencedatac, Prevalencedataa, T,0.3))
%% MC Optimization
parameter1 = linspace(0.04, 0.29, 50);
parameter2 = linspace(0.3, 0.57, 50);
firstparameter = [parameter2 flip(parameter1)];
...
j = [0.00027 0.01 0.01 0.00005 0.1 0.074 0.2 0.0067 0.00018 0.000033 0.0067];
%this is where you change the parameter (delete it)
for i = 1:length_of_beta
    Initial_Guess = j;
    disp("Iteration value d%", i)
    ...
    [EstimatedParameters,fval,exitflag]=fminsearchbnd(@(Initial_Guess)err_in_data_SIR(Initial_Guess,Prevalencedatac, Prevalencedataa, T,fixp),Initial_Guess, Lowerbounds, Upperbounds, options);
...
figure
plot(firstparameter([100:-1:51,1:50]), function_values([100:-1:51,1:50]))
hold on 
yline(threshold, 'r','Threshold')
yline(thresholdup, 'r', 'Threshold max' )
title('Profile Likelihood {\epsilon_{C}}')
hold off 
