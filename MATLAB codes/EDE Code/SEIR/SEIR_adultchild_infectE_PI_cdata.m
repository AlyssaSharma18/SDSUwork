...
%Initial Guess
Fitted_Parameters=[0.2 0.000001 0.1 0.00000002];%[0.0000005 0.1 0.05 0.00000001 0.02 0.01 0.04 0.02 0.2 0.000001 0.1 0.00000002]; %betaaa, betacc, betaac, betaca, gammac, gammaa, epsilonc, epsilona
%Peak at day 109.
...
for IterationLevels = 1:6
    ...
    % better not have them, or only print out every 100 iterations 
        Noise  = NoiseLevel*y_est(tspan(:),5)' + NoiseLevel*y_est(tspan(:),6)';
        Prevalencedata = normrnd(y_est(tspan(:),5)'+y_est(tspan(:),6)', Noise)'; %Equation from step 2. Section 4.1. Use normal distribution.
        ...
        while (value~=0)
            Prevalencedata = normrnd(y_est(tspan(:),5)'+y_est(tspan(:),6)', Noise)';
            value=sum(Prevalencedata(:)<-0.001);
        end
        ...
        [EstimatedParameters,fval,exitflag]=fmincon(@(Initial_Guess)err_in_dataSEIR_GLS_AdultChild_infectE(Initial_Guess,Prevalencedata, T),Initial_Guess,[],[],[],[],Lowerbounds, Upperbounds, [], options);
    ...
    fprintf("Below are the ARE values for noise level %10.10f \n", NoiseLevel)
...
function [dx]  = SEIR_model_states_AdultChild_infectE(t,x,z)
    ...
    betaaa = 0.00019; %5*10^-7 to 2*10^-5
    betacc = 0.1; %0.1 to 1
    betaac = 0.1; %.05 to 1 <betacc
    betaca = 0.0000001; % 1*10^-8 to 5*10^-7
    gammac = 0.02; %.02 to 1
    gammaa = 0.01; % .01 to 1
    epsilonc = 0.04; % .04 to 1
    epsilona = 0.02; % .02 to 1
    xicc = z(1); %0.2
    xiaa = z(2); %0.000001
    xica = z(3); %0.1
    xiac = z(4);%0.00000002;
...
function  error_in_data = err_in_dataSEIR_GLS_AdultChild_infectE(par, data, T)
