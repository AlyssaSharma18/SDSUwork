...
%Initial Guess
Fitted_Parameters=[0.00027 0.1 0.074 0.3 0.2 0.0002];%[0.0000005 0.1 0.05 0.00000001 0.02 0.01 0.04 0.02 0.2 0.000001 0.1 0.00000002]; %betaaa, betacc, betaac, betaca, gammac, gammaa, epsilonc, epsilona
%Peak at day 109.
...
function [dx]  = SEIR_model_states_AdultChild_infectE(t,x,z)
    ...
    beta = z(1);
    betaaa = beta; %0.00027; %z(1); %5*10^-7 to 2*10^-5
    ...
    gammac = z(2); %0.1; %.02 to 1
    gammaa = z(3); %0.074; % .01 to 1
    epsilonc = z(4); %0.3; % .04 to 1
    epsilona = z(5); %0.2; % .02 to 1 
    xi = z(6); %tentatively 0.0002
    xicc = (3/2)*250*xi;
    xiaa = xi; 
    xica = (3/4)*xi; 
    xiac = (1/3)*750*xi; 
