...
%Initial Guess
Fitted_Parameters=[0.00019 0.02 0.01 0.04 0.02 0.2];%[0.0000005 0.1 0.05 0.00000001 0.02 0.01 0.04 0.02 0.2 0.000001 0.1 0.00000002]; %betaaa, betacc, betaac, betaca, gammac, gammaa, epsilonc, epsilona
%Peak at day 109.
...
for IterationLevels = 1:6
    ...
    % better not have them, or only print out every 100 iterations 
        ...
        [EstimatedParameters,fval,exitflag]=fmincon(@(Initial_Guess)err_in_dataSEIR_GLS_AdultChild_infectE(Initial_Guess,Prevalencedatac,Prevalencedataa, T),Initial_Guess,[],[],[],[],Lowerbounds, Upperbounds, [], options);
...
function [dx]  = SEIR_model_states_AdultChild_infectE(t,x,z)
    ...
    beta = z(1);
    betaaa = beta; %0.00019; %z(1); %5*10^-7 to 2*10^-5
    betacc = (3/2)*250*beta; %0.1; %z(2); %0.1 to 1
    betaac = (1/3)*750*beta; %0.1; %z(3); %.05 to 1 <betacc
    betaca = (3/4)*beta; %0.0000001; %z(4); % 1*10^-8 to 5*10^-7
    gammac = z(2); %0.02; %.02 to 1
    gammaa = z(3); %0.01; % .01 to 1
    epsilonc = z(4); %0.04; % .04 to 1
    epsilona = z(5); %0.02; % .02 to 1
    xi = z(6); 
    xicc = (3/2)*250*xi;% 0.2;
    xiaa = xi; %0.000001;
    xica = (3/4)*xi; %0.1;
    xiac = (1/3)*750*xi; %0.00000002;
...
function  error_in_data = err_in_dataSEIR_GLS_AdultChild_infectE(par, datac, dataa, T)
